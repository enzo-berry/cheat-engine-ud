...
begin
  shellexecute(0,'open',pchar('https://www.patreon.com/llamatools'),nil,nil,sw_maximize);
end;

procedure TAbout.Label8Click(Sender: TObject);
begin
  ShellExecute(0, pchar('open'),pchar('https://llamatools.org/'), pchar(''),pchar(''), SW_MAXIMIZE	);
end;

procedure TAbout.Label9Click(Sender: TObject);
begin
  ShellExecute(0, pchar('open'),pchar('http://forum.llamatools.org/'), pchar(''),pchar(''), SW_MAXIMIZE	);
end;
